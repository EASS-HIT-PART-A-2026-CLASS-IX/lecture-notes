...
## What You’ll Build
- `movie_service/app/models.py` holding SQLModel definitions (`Movie`, `MovieCreate`, `MovieRead`) plus a simple `MovieUpdate` stub the Session 06 UI work can lean on when you add edit forms.
...
- Emphasize architecture boundaries: **Session 03 kept storage behind a repository interface** for exactly this reason. All new database touch points stay inside `repository_db.py` / `database.py`, so future swaps (Session 05’s PostgreSQL upgrade, Session 06’s Streamlit UI, Session 07’s React + diagnostics, Session 08’s AI tools) stay surgical.
...
- Add helper methods for `update` so the Streamlit forms in Session 06 (and future clients) can introduce PUT/PATCH quickly.
...
class MovieRepository:
    ...
    def list(self) -> list[Movie]:
        ...
        Returns list[Movie] consistently with Session 03.
        We convert SQLModel's Sequence to list for interface consistency.
        """
        return list(self.session.exec(select(Movie)).all())
    ...
    def delete_all(self) -> int:
        """Remove all movies and return count deleted.
        
        Added for Session 06's Typer CLI wipe command.
        """
...
## Session 05 Preview – What Changes:

⚠️ **Moving from SQLite to PostgreSQL:**

| Component | Session 04 (Current SQLite) | Session 05 (PostgreSQL) | Changes? |
|-----------|----------------------------|------------------------|----------|
| `database.py` | `sqlite:///data/movies.db` | `postgresql+psycopg://...` | ✅ URL only |
| `config.py` | DATABASE_URL in .env | Add pool settings | ✅ Add fields |
| `models.py` | SQLModel classes | SAME classes | ✅ No changes! |
| `repository_db.py` | SQLite queries | SAME code | ✅ No changes! |
| `alembic/env.py` | Points to SQLite | Points to Postgres | ✅ URL swap |
| Tests | Temp SQLite files | Temp Postgres DBs | ⚠️ New fixtures |
| Dev workflow | `uv run uvicorn` | `docker compose up` first | ⚠️ Add step |

**What stays the same:**
- All route handlers
- All model definitions
- Repository method signatures
- Test assertions (only fixtures change)

**What changes:**
- `.env` gets Postgres connection string
- `docker-compose.yml` added for local Postgres
- Test fixtures create/drop Postgres databases
- Health check added for connection monitoring

**Action items before Session 05:**
1. Install Docker Desktop and verify `docker compose version`
2. Commit Session 04 code and tag as `session-04-complete`
3. Run `uv run pytest -v` and confirm all tests pass
4. Note your current `.env` settings (you'll update them)
...
- Revisit Dockerfile ahead of Session 06 so the Streamlit container (and any SQLite volume mounts) can share the same persisted data.
...
If any box is unchecked, pair with a mentor before Session 06 so the Streamlit UI in that session sits on a stable API + database base.
