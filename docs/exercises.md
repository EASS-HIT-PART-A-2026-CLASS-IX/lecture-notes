...
- **Goal:** Finish a tidy local product by joining the EX1 API and EX2 interface, adding one meaningful improvement, and writing the documentation you wish every teammate handed you. Everything must run with `uv run` or `python -m`—no Docker, cloud, or security features required.
...
- **Required Pieces:**
  - One Git repository containing the API, the interface, and a `docs/EX3-notes.md` playbook with run commands.
